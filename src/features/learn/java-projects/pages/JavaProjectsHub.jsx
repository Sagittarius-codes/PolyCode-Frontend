...
import {
  JAVA_PROJECTS_CHAPTERS,
  JAVA_PROJECTS_LESSONS,
  JAVA_PROJECTS_TOTAL_XP,
} from "../data/javaProjectsCurriculum";
import useJavaProjectsProgress from "../hooks/useJavaProjectsProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/java-projects";

const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["task-manager"],
    color: "#06b6d4",
    summary: "Build a console Task Manager — classes, Lists, filtering, and sorting.",
  },
  {
    level: "Intermediate",
    chapters: ["library-system"],
    color: "#3b82f6",
    summary: "Build a Library System — inheritance and custom exceptions.",
  },
  {
    level: "Advanced",
    chapters: ["bank-system"],
    color: "#f59e0b",
    summary: "Build a Bank Account System — transactions, rollback, and persistence.",
  },
  {
    level: "Pro",
    chapters: ["ecommerce-system"],
    color: "#8b5cf6",
    summary: "Build a Mini E-Commerce backend — DAO, services, and stream analytics.",
  },
];
...
export default function JavaProjectsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useJavaProjectsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = JAVA_PROJECTS_LESSONS.filter(
    (l) => progress[l.id],
  ).reduce((sum, l) => sum + l.xp, 0);
  const pct =
    Math.round((completedCount / JAVA_PROJECTS_LESSONS.length) * 100) || 0;

  const nextLesson =
    JAVA_PROJECTS_LESSONS.find((l) => !progress[l.id]) ||
    JAVA_PROJECTS_LESSONS[0];
  const resumeLesson =
    JAVA_PROJECTS_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = JAVA_PROJECTS_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => JAVA_PROJECTS_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return JAVA_PROJECTS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub js-fundamentals-hub">
      ...
      <div className="oops-hero js-fundamentals-hero">
        ...
        <div className="oops-hero-badge">JAVA · PROJECTS</div>
        <h1 className="oops-hero-title">
          Java
          <br />
          <span className="oops-hero-accent" style={{ color: "#f97316" }}>
            Projects
          </span>
        </h1>
        <p className="oops-hero-sub">
          Apply everything you've learned to build 4 real, growing projects —
          a task manager, a library system, a bank system, and an
          e-commerce backend — compiled by{" "}
          <code>javac</code>.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${JAVA_PROJECTS_LESSONS.length} lessons · ${earnedXP}/${JAVA_PROJECTS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${JAVA_PROJECTS_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{JAVA_PROJECTS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{JAVA_PROJECTS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{JAVA_PROJECTS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <div className="oops-path-overview">
        {JAVA_PROJECTS_CHAPTERS.map((chapter, index) => {
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {JAVA_PROJECTS_CHAPTERS.length} chapters ·{" "}
            {JAVA_PROJECTS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = JAVA_PROJECTS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={JAVA_PROJECTS_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        courseName="Java Projects"
        totalLessons={JAVA_PROJECTS_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={JAVA_PROJECTS_TOTAL_XP}
      />
    </div>
  );
}
