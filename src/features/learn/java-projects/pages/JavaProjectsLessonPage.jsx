...
const READ_GATE_PREFIX = "java_projects";

export default function JavaProjectsLessonPage() {
  ...
  const lesson = JAVA_PROJECTS_LESSONS.find((l) => l.id === lessonId);
  const lessonIdx = JAVA_PROJECTS_LESSONS.findIndex(
    (l) => l.id === lessonId,
  );
  const prev = JAVA_PROJECTS_LESSONS[lessonIdx - 1];
  const next = JAVA_PROJECTS_LESSONS[lessonIdx + 1];
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        {/* ── Content ── */}
        <LessonContentShell
          storageKey={`java-projects:${lessonId}`}
