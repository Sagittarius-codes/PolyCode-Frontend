...
const LEARNING_PATH = [
  {
    ...
    chapters: ["ch-0"],
    color: "#8e44ad",
    summary: "Stack vs heap, memory segments, and reading addresses with %p.",
  },
  {
    level: "Intermediate",
    chapters: ["ch-1", "ch-2"],
    color: "#7d3c98",
    summary: "malloc, free, calloc, realloc, and NULL checks.",
  },
  {
    level: "Advanced",
    chapters: ["ch-3"],
    color: "#6c3483",
    summary: "Memory leaks, dangling pointers, and buffer overflows.",
  },
  {
    level: "Pro",
    chapters: ["ch-4"],
    color: "#5b2c6f",
    summary: "Dynamic arrays, safe free pattern, and heap string builder capstone.",
  },
];
...
export default function CMemoryManagementHub() {
  ...
  const earnedXP = C_MEMORY_MANAGEMENT_LESSONS.filter((l) => progress[l.id]).reduce(
    (sum, l) => sum + l.xp,
    0,
  );
  const pct =
    Math.round((completedCount / C_MEMORY_MANAGEMENT_LESSONS.length) * 100) || 0;
  const nextLesson =
    C_MEMORY_MANAGEMENT_LESSONS.find((l) => !progress[l.id]) || C_MEMORY_MANAGEMENT_LESSONS[0];
  const resumeLesson =
    C_MEMORY_MANAGEMENT_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = C_MEMORY_MANAGEMENT_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => C_MEMORY_MANAGEMENT_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return C_MEMORY_MANAGEMENT_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub c-hub">
      <div className="oops-hero c-hero" style={{ borderColor: "#8e44ad" }}>
        <Link
          to="/language/C"
        ...
        <div className="oops-hero-badge">C · MEMORY TRACK</div>
        <h1 className="oops-hero-title">
          C Memory
          <br />
          <span className="oops-hero-accent" style={{ color: "#8e44ad" }}>
            Management
          </span>
        </h1>
        <p className="oops-hero-sub">Stack vs heap, malloc, calloc, realloc, free — manage memory safely and avoid leaks.</p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${C_MEMORY_MANAGEMENT_LESSONS.length} lessons · ${earnedXP}/${C_MEMORY_MANAGEMENT_TOTAL_XP} XP`
                  : `Sign in to track progress · ${C_MEMORY_MANAGEMENT_LESSONS.length} lessons`}
              </span>
            ...
            <div className="oops-xp-track">
              <div
                ...
                style={{ width: isAuthenticated ? `${pct}%` : "0%", backgroundColor: "#8e44ad" }}
              />
            </div>
          </div>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C Memory Management" : "Start C Memory Management"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search malloc, heap, free..."
              aria-label="Search C Memory Management lessons"
            />
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>{completedCount}/{C_MEMORY_MANAGEMENT_LESSONS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>{completedChapters}/{C_MEMORY_MANAGEMENT_CHAPTERS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>{earnedXP}/{C_MEMORY_MANAGEMENT_TOTAL_XP}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {C_MEMORY_MANAGEMENT_CHAPTERS.length} chapters ·{" "}
      {C_MEMORY_MANAGEMENT_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = C_MEMORY_MANAGEMENT_CHAPTERS.filter((ch) =>
