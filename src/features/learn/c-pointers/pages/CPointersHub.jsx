...
const LEARNING_PATH = [
  {
    ...
    chapters: ["ch-0"],
    color: "#e74c3c",
    summary: "What a pointer is, dereferencing with *, and NULL.",
  },
  {
    level: "Intermediate",
    chapters: ["ch-1", "ch-2"],
    color: "#c0392b",
    summary: "Pointer arithmetic, incrementing, and arrays via pointers.",
  },
  {
    level: "Advanced",
    chapters: ["ch-3"],
    color: "#a93226",
    summary: "char* vs char[], string functions, and array of strings.",
  },
  {
    level: "Pro",
    chapters: ["ch-4"],
    color: "#7b241c",
    summary: "Double pointers, pointers to structs, and linked list capstone.",
  },
];
...
export default function CPointersHub() {
  ...
  const earnedXP = C_POINTERS_LESSONS.filter((l) => progress[l.id]).reduce(
    (sum, l) => sum + l.xp,
    0,
  );
  const pct =
    Math.round((completedCount / C_POINTERS_LESSONS.length) * 100) || 0;
  const nextLesson =
    C_POINTERS_LESSONS.find((l) => !progress[l.id]) || C_POINTERS_LESSONS[0];
  const resumeLesson =
    C_POINTERS_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = C_POINTERS_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => C_POINTERS_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return C_POINTERS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub c-hub">
      <div className="oops-hero c-hero" style={{ borderColor: "#e74c3c" }}>
        <Link
          to="/language/C"
        ...
        <div className="oops-hero-badge">C · POINTERS TRACK</div>
        <h1 className="oops-hero-title">
          C
          <br />
          <span className="oops-hero-accent" style={{ color: "#e74c3c" }}>
            Pointers
          </span>
        </h1>
        <p className="oops-hero-sub">Addresses, dereferencing, pointer arithmetic, arrays, strings, and double pointers — unlock C's real power.</p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${C_POINTERS_LESSONS.length} lessons · ${earnedXP}/${C_POINTERS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${C_POINTERS_LESSONS.length} lessons`}
              </span>
            ...
            <div className="oops-xp-track">
              <div
                ...
                style={{ width: isAuthenticated ? `${pct}%` : "0%", backgroundColor: "#e74c3c" }}
              />
            </div>
          </div>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C Pointers" : "Start C Pointers"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search addresses, arrays, NULL..."
              aria-label="Search C Pointers lessons"
            />
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>{completedCount}/{C_POINTERS_LESSONS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>{completedChapters}/{C_POINTERS_CHAPTERS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>{earnedXP}/{C_POINTERS_TOTAL_XP}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {C_POINTERS_CHAPTERS.length} chapters ·{" "}
      {C_POINTERS_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = C_POINTERS_CHAPTERS.filter((ch) =>
