...
const READ_GATE_PREFIX = "java_advanced";

export default function JavaAdvancedLessonPage() {
  ...
  const lesson = JAVA_ADVANCED_LESSONS.find((l) => l.id === lessonId);
  const lessonIdx = JAVA_ADVANCED_LESSONS.findIndex(
    (l) => l.id === lessonId,
  );
  const prev = JAVA_ADVANCED_LESSONS[lessonIdx - 1];
  const next = JAVA_ADVANCED_LESSONS[lessonIdx + 1];
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <OopsSidebar
        ...
        progress={progress}
        chapters={JAVA_ADVANCED_CHAPTERS}
        basePath={BASE_PATH}
        title="Java Advanced"
      />
      ...
      <div className="oops-lesson-main">
        ...
        {/* ── Content ── */}
        <LessonContentShell
          storageKey={`java-advanced:${lessonId}`}
