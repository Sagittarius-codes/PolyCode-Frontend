...
export default function SqlViewsHub() {
  ...
  const earnedXP = SQLVIEWS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / SQLVIEWS_LESSONS.length) * 100) || 0;
  const nextLesson =
    SQLVIEWS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    SQLVIEWS_LESSONS[0];
  const resumeLesson =
    SQLVIEWS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = SQLVIEWS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => SQLVIEWS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return SQLVIEWS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub cpp-fundamentals-hub">
      <div className="oops-hero cpp-fundamentals-hero">
        ...
        <p className="oops-hero-sub">
          Master SQL Views with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${SQLVIEWS_LESSONS.length} lessons · ${earnedXP}/${SQLVIEWS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${SQLVIEWS_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{SQLVIEWS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{SQLVIEWS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{SQLVIEWS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {SQLVIEWS_CHAPTERS.length} chapters · {" "}
      {SQLVIEWS_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = SQLVIEWS_CHAPTERS.filter((ch) =>
