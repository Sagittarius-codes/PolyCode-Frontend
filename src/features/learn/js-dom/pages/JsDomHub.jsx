...
export default function JsDomHub() {
  ...
  const earnedXP = JS_DOM_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct = Math.round((completedCount / JS_DOM_LESSONS.length) * 100) || 0;
  const nextLesson =
    JS_DOM_LESSONS.find((lesson) => !progress[lesson.id]) || JS_DOM_LESSONS[0];
  const resumeLesson =
    JS_DOM_LESSONS.find((lesson) => lesson.id === lastLessonId) || nextLesson;
  const completedChapters = JS_DOM_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => JS_DOM_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return JS_DOM_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub js-dom-hub">
      <div className="oops-hero js-dom-hero">
        <Link
        ...
        <div className="oops-hero-badge">JAVASCRIPT · DOM TRACK</div>
        <h1 className="oops-hero-title">
          JavaScript
          <br />
          <span className="oops-hero-accent">DOM Course</span>
        </h1>
        <p className="oops-hero-sub">
          Learn how JavaScript controls the browser page, selects and updates content,
          responds to user actions, and builds fast interactive DOM-driven UI.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${JS_DOM_LESSONS.length} lessons · ${earnedXP}/${JS_DOM_TOTAL_XP} XP`
                  : `Sign in to track progress · ${JS_DOM_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume DOM course" : "Start DOM course"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a DOM topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search DOM, events, querySelector..."
              aria-label="Search DOM lessons"
            />
            <div className="oops-filter-tabs" aria-label="Filter DOM lessons">
              {[
                ["all", "All"],
                ["todo", "To do"],
                ["done", "Done"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{JS_DOM_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{JS_DOM_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{JS_DOM_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
