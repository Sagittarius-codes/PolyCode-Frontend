...
const READ_GATE_PREFIX = "js_dom";

export default function JsDomLessonPage() {
  ...
  const lesson = JS_DOM_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = JS_DOM_LESSONS.findIndex((item) => item.id === lessonId);
  const prev = JS_DOM_LESSONS[lessonIdx - 1];
  const next = JS_DOM_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>JavaScript DOM lesson not found.</p>
  ...
  const earnedXP = JS_DOM_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="JavaScript DOM"
            syncLabel={
              isAuthenticated
                ? "JavaScript DOM progress saved to your account"
