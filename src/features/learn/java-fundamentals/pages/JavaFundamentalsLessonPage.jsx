...
import JavaCodeChallenge from "../components/JavaCodeChallenge";
import {
...
const READ_GATE_PREFIX = "java_fundamentals";

export default function JavaFundamentalsLessonPage() {
  ...
  const lesson = JAVA_FUNDAMENTALS_LESSONS.find((l) => l.id === lessonId);
  const lessonIdx = JAVA_FUNDAMENTALS_LESSONS.findIndex(
    (l) => l.id === lessonId,
  );
  const prev = JAVA_FUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = JAVA_FUNDAMENTALS_LESSONS[lessonIdx + 1];
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        {/* ── Content ── */}
        <LessonContentShell
          storageKey={`java-fundamentals:${lessonId}`}
