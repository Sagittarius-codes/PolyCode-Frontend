...
import {
  QUANTUM_COMPUTING_FUNDAMENTALS_CHAPTERS,
  QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS,
  QUANTUM_COMPUTING_FUNDAMENTALS_TOTAL_XP,
} from "../data/quantumComputingFundamentalsCurriculum";
import useQuantumComputingFundamentalsProgress from "../hooks/useQuantumComputingFundamentalsProgress";
import CourseCertificate from "../../shared/CourseCertificate";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import LearnChapterIcon from "../../shared/LearnChapterIcon";

const BASE_PATH = "/learn/quantum-computing-fundamentals";

const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["quantum-foundations"],
    color: "#6366f1",
    summary: "What quantum computing is, and how qubits differ from classical bits.",
  },
  {
    level: "Intermediate",
    chapters: ["quantum-core-concepts"],
    color: "#8b5cf6",
    summary: "Superposition and entanglement — the two ideas everything else builds on.",
  },
  {
    level: "Advanced",
    chapters: ["quantum-circuits"],
    color: "#a855f7",
    summary: "Gates, circuits, and how measurement turns a quantum state into an answer.",
  },
];
...
export default function QuantumComputingFundamentalsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useQuantumComputingFundamentalsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct =
    Math.round(
      (completedCount / QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.length) * 100,
    ) || 0;
  const nextLesson =
    QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS[0];
  const resumeLesson =
    QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = QUANTUM_COMPUTING_FUNDAMENTALS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub matplotlib-hub">
      <div className="oops-hero matplotlib-hero">
        ...
        <p className="oops-hero-sub">
          A structured path from "what is a qubit?" to superposition,
          entanglement, gates, and measurement — 3 chapters, 6 lessons, and
          light hands-on checks before Algorithms and Projects.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.length} lessons · ${earnedXP}/${QUANTUM_COMPUTING_FUNDAMENTALS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume Quantum" : "Start Quantum"}
            </button>
          </div>
        </div>
      </div>
      ...
      <section className="matplotlib-prerequisites" aria-label="What comes next">
        <div className="matplotlib-prerequisites-head">
          ...
          <small>Continue your Quantum path on PolyCode</small>
        </div>
        <div className="matplotlib-prerequisites-grid">
          <Link to="/learn/quantum-mechanics-for-programmers" className="matplotlib-prereq-card">
            <strong>Quantum Mechanics</strong>
            <p>The physics and linear algebra behind the circuits you just built.</p>
          </Link>
          <Link to="/learn/quantum-algorithms" className="matplotlib-prereq-card">
            <strong>Quantum Algorithms</strong>
            <p>Deutsch-Jozsa, Grover's search, and Shor's algorithm.</p>
          </Link>
          <Link to="/learn/quantum-programming-projects" className="matplotlib-prereq-card">
            <strong>Quantum Projects</strong>
            <p>Build real circuits and hybrid classical-quantum programs.</p>
          </Link>
          <Link
            to={`${BASE_PATH}/lesson/qc-5`}
            className="matplotlib-prereq-card matplotlib-prereq-cheat"
          >
            <strong>Measurement lesson</strong>
            <p>Revisit how quantum programs are actually read out.</p>
          </Link>
        </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a Quantum topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search qubits, gates, entanglement..."
              aria-label="Search Quantum lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Quantum lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{QUANTUM_COMPUTING_FUNDAMENTALS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{QUANTUM_COMPUTING_FUNDAMENTALS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {QUANTUM_COMPUTING_FUNDAMENTALS_CHAPTERS.length} chapters ·{" "}
            {QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = QUANTUM_COMPUTING_FUNDAMENTALS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={QUANTUM_COMPUTING_FUNDAMENTALS_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        ...
        totalLessons={QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={QUANTUM_COMPUTING_FUNDAMENTALS_TOTAL_XP}
      />
    </div>
  );
}
