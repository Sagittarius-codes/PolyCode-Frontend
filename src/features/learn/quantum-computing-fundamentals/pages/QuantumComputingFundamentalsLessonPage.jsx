...
const READ_GATE_PREFIX = "quantum_computing_fundamentals";

export default function QuantumComputingFundamentalsLessonPage() {
  ...
  const lesson = QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.find(
    (item) => item.id === lessonId,
  );
  const lessonIdx = QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = QUANTUM_COMPUTING_FUNDAMENTALS_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Quantum lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to Quantum Computing Fundamentals
        </button>
      </div>
    );
  }
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Quantum Computing Fundamentals"
            syncLabel={
              isAuthenticated
                ? "Quantum progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`quantum-computing-fundamentals:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Quantum Computing Fundamentals`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
