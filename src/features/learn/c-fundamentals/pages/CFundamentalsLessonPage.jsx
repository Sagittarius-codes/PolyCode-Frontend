...
import CFundamentalsCodeChallenge from "../components/CFundamentalsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "c_fundamentals";

export default function CFundamentalsLessonPage() {
  ...
  const lesson = C_FUNDAMENTALS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = C_FUNDAMENTALS_LESSONS.findIndex((item) => item.id === lessonId);
  const prev = C_FUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = C_FUNDAMENTALS_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>C Fundamentals lesson not found.</p>
  ...
  const earnedXP = C_FUNDAMENTALS_LESSONS.filter((item) => progress[item.id]).reduce(
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            ...
            syncLabel={
              isAuthenticated
                ? "C Fundamentals progress saved to your account"
