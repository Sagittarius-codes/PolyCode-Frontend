...
const READ_GATE_PREFIX = "php_forms";

export default function PhpFormsLessonPage() {
  ...
  const lesson = PHP_FORMS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = PHP_FORMS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = PHP_FORMS_LESSONS[lessonIdx - 1];
  const next = PHP_FORMS_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>PHP Forms lesson not found.</p>
  ...
  const earnedXP = PHP_FORMS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PHP Forms"
            syncLabel={
              isAuthenticated
                ? "PHP Forms progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`php-forms:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PHP Forms`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
