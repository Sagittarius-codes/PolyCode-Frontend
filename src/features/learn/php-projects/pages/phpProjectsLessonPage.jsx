...
const READ_GATE_PREFIX = "php_projects";

export default function PhpProjectsLessonPage() {
  ...
  const lesson = PHP_PROJECTS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = PHP_PROJECTS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = PHP_PROJECTS_LESSONS[lessonIdx - 1];
  const next = PHP_PROJECTS_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>PHP Projects lesson not found.</p>
  ...
  const earnedXP = PHP_PROJECTS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PHP Projects"
            syncLabel={
              isAuthenticated
                ? "PHP Projects progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`php-projects:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PHP Projects`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
