...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["routing-controllers"],
    color: "#ef4444",
    summary: "MVC overview, defining routes, controllers, and route parameters.",
  },
  {
    level: "Intermediate",
    chapters: ["blade-templates"],
    color: "#f59e0b",
    summary: "Blade basics, layouts, passing data to views, and components.",
  },
  {
    level: "Advanced",
    chapters: ["eloquent-orm"],
    color: "#3b82f6",
    summary: "Models, migrations, querying, relationships, and mass assignment.",
  },
  {
    level: "Pro",
    chapters: ["middleware-modern-laravel"],
    color: "#8b5cf6",
    summary: "Middleware, form validation, route model binding, and a full CRUD flow.",
  },
];
...
export default function LaravelBasicsHub() {
  ...
  const earnedXP = LARAVEL_BASICS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / LARAVEL_BASICS_LESSONS.length) * 100) || 0;
  const nextLesson =
    LARAVEL_BASICS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    LARAVEL_BASICS_LESSONS[0];
  const resumeLesson =
    LARAVEL_BASICS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = LARAVEL_BASICS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => LARAVEL_BASICS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return LARAVEL_BASICS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub laravel-basics-hub">
      <div className="oops-hero laravel-basics-hero">
        <Link
          to="/language/PHP"
        ...
        <div className="oops-hero-badge">PHP · LARAVEL</div>
        <h1 className="oops-hero-title">
          PHP
          <br />
          <span className="oops-hero-accent" style={{ color: "#ef4444" }}>Laravel</span>
        </h1>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${LARAVEL_BASICS_LESSONS.length} lessons · ${earnedXP}/${LARAVEL_BASICS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${LARAVEL_BASICS_LESSONS.length} lessons`}
              </span>
            ...
            <div className="oops-xp-track">
              <div
                ...
                style={{ width: isAuthenticated ? `${pct}%` : "0%", backgroundColor: "#ef4444" }}
              />
            </div>
          </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a Laravel topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search routes, Blade, Eloquent..."
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{LARAVEL_BASICS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{LARAVEL_BASICS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{LARAVEL_BASICS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {LARAVEL_BASICS_CHAPTERS.length} chapters ·{" "}
            {LARAVEL_BASICS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = LARAVEL_BASICS_CHAPTERS.filter((ch) =>
