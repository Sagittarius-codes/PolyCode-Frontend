...
import SqlQueriesCodeChallenge from "../components/SqlQueriesCodeChallenge";
import {
  SQLQUERIES_CHAPTERS,
  SQLQUERIES_LESSONS,
  SQLQUERIES_TOTAL_XP,
...
const READ_GATE_PREFIX = "sqlqueries";

export default function SqlQueriesLessonPage() {
  ...
  const lesson = SQLQUERIES_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SQLQUERIES_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SQLQUERIES_LESSONS[lessonIdx - 1];
  const next = SQLQUERIES_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = SQLQUERIES_LESSONS.filter((item) => progress[item.id]).reduce(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sqlqueries:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — SQL Queries`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
