...
import {
  SQLAGGREGATEFUNCTIONS_CHAPTERS,
  SQLAGGREGATEFUNCTIONS_LESSONS,
  SQLAGGREGATEFUNCTIONS_TOTAL_XP,
} from "../data/sqlAggregateFunctionsCurriculum";
import useSqlAggregateFunctionsProgress from "../hooks/useSqlAggregateFunctionsProgress";
...
export default function SqlAggregateFunctionsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useSqlAggregateFunctionsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = SQLAGGREGATEFUNCTIONS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / SQLAGGREGATEFUNCTIONS_LESSONS.length) * 100) || 0;
  const nextLesson =
    SQLAGGREGATEFUNCTIONS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    SQLAGGREGATEFUNCTIONS_LESSONS[0];
  const resumeLesson =
    SQLAGGREGATEFUNCTIONS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = SQLAGGREGATEFUNCTIONS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => SQLAGGREGATEFUNCTIONS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return SQLAGGREGATEFUNCTIONS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub cpp-fundamentals-hub">
      <div className="oops-hero cpp-fundamentals-hero">
        ...
        <p className="oops-hero-sub">
          Master SQL Aggregate Functions with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${SQLAGGREGATEFUNCTIONS_LESSONS.length} lessons · ${earnedXP}/${SQLAGGREGATEFUNCTIONS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${SQLAGGREGATEFUNCTIONS_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{SQLAGGREGATEFUNCTIONS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{SQLAGGREGATEFUNCTIONS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{SQLAGGREGATEFUNCTIONS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {SQLAGGREGATEFUNCTIONS_CHAPTERS.length} chapters · {" "}
      {SQLAGGREGATEFUNCTIONS_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = SQLAGGREGATEFUNCTIONS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={SQLAGGREGATEFUNCTIONS_CHAPTERS}
        progress={progress}
