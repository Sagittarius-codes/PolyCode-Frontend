...
import SqlAggregateFunctionsCodeChallenge from "../components/SqlAggregateFunctionsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "sqlaggregatefunctions";

export default function SqlAggregateFunctionsLessonPage() {
  ...
  const lesson = SQLAGGREGATEFUNCTIONS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SQLAGGREGATEFUNCTIONS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SQLAGGREGATEFUNCTIONS_LESSONS[lessonIdx - 1];
  const next = SQLAGGREGATEFUNCTIONS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "SQL Aggregate Functions",
    language: "SQL",
    lesson,
    ...
    return (
      <div className="oops-not-found">
        ...
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to SQL Aggregate Functions
        </button>
      </div>
    );
  }
  ...
  const earnedXP = SQLAGGREGATEFUNCTIONS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="SQL Aggregate Functions"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sqlaggregatefunctions:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — SQL Aggregate Functions`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <SqlAggregateFunctionsCodeChallenge
