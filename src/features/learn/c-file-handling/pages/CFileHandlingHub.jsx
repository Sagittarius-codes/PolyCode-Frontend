...
const LEARNING_PATH = [
  {
    ...
    chapters: ["ch-0"],
    color: "#27ae60",
    summary: "FILE* pointer, fopen/fclose modes, and error handling.",
  },
  {
    level: "Intermediate",
    chapters: ["ch-1", "ch-2"],
    color: "#1e8449",
    summary: "fgetc, fgets, fscanf for reading; fputc, fputs, fprintf for writing.",
  },
  {
    level: "Advanced",
    chapters: ["ch-3"],
    color: "#196f3d",
    summary: "fseek, ftell, rewind, feof, and ferror.",
  },
  {
    level: "Pro",
    chapters: ["ch-4"],
    color: "#145a32",
    summary: "CSV parsing, binary file store, and note saver capstone.",
  },
];
...
export default function CFileHandlingHub() {
  ...
  const earnedXP = C_FILE_HANDLING_LESSONS.filter((l) => progress[l.id]).reduce(
    (sum, l) => sum + l.xp,
    0,
  );
  const pct =
    Math.round((completedCount / C_FILE_HANDLING_LESSONS.length) * 100) || 0;
  const nextLesson =
    C_FILE_HANDLING_LESSONS.find((l) => !progress[l.id]) || C_FILE_HANDLING_LESSONS[0];
  const resumeLesson =
    C_FILE_HANDLING_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = C_FILE_HANDLING_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => C_FILE_HANDLING_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return C_FILE_HANDLING_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub c-hub">
      <div className="oops-hero c-hero" style={{ borderColor: "#27ae60" }}>
        <Link
          to="/language/C"
        ...
        <div className="oops-hero-badge">C · FILE I/O TRACK</div>
        <h1 className="oops-hero-title">
          C File
          <br />
          <span className="oops-hero-accent" style={{ color: "#27ae60" }}>
            Handling
          </span>
        </h1>
        <p className="oops-hero-sub">Open, read, write, seek, and close files — persist data beyond your program's lifetime.</p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${C_FILE_HANDLING_LESSONS.length} lessons · ${earnedXP}/${C_FILE_HANDLING_TOTAL_XP} XP`
                  : `Sign in to track progress · ${C_FILE_HANDLING_LESSONS.length} lessons`}
              </span>
            ...
            <div className="oops-xp-track">
              <div
                ...
                style={{ width: isAuthenticated ? `${pct}%` : "0%", backgroundColor: "#27ae60" }}
              />
            </div>
          </div>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C File Handling" : "Start C File Handling"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search fopen, fread, fwrite..."
              aria-label="Search C File Handling lessons"
            />
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>{completedCount}/{C_FILE_HANDLING_LESSONS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>{completedChapters}/{C_FILE_HANDLING_CHAPTERS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>{earnedXP}/{C_FILE_HANDLING_TOTAL_XP}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {C_FILE_HANDLING_CHAPTERS.length} chapters ·{" "}
      {C_FILE_HANDLING_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = C_FILE_HANDLING_CHAPTERS.filter((ch) =>
