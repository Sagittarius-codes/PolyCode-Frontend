...
export default function RustFundamentalsCodeChallenge({
  challenge,
  accentColor,
  isCompleted,
  onComplete,
  ...
  function runTests() {
    ...
    setOutput({
      status: "running",
      stdout: "Running Rust compilation checks…",
    });

    window.setTimeout(async () => {
      let expectedOutput = "";
      try {
        const expectedRun = await runRustCode(challenge.solutionCode);
        expectedOutput = formatRustOutput(expectedRun.result);
      } catch {
        expectedOutput = "";
      }
      ...
      let runPayload;
      try {
        runPayload = await runRustCode(code);
      } catch (error) {
        setResults({
          passed: false,
          tests: [
            {
              id: "runtime",
              label: "Rust compiles and runs without errors",
              passed: false,
              hint: error.message || "Could not execute Rust code.",
            },
      ...
      const runtimeError = getRustRuntimeError(runResult);
      const stdout = formatRustOutput(runResult);
      ...
      if (allPassed) {
        ...
        if (!isCompleted) {
          Promise.resolve(onComplete()).catch((error) => {
            console.error("Unable to save Rust lesson progress:", error);
          });
        }
      }
    ...
    <div className="oops-challenge">
      ...
      <div className="oops-editor-panel">
        <div className="oops-editor-topbar">
          <span className="oops-editor-lang">Rust · main.rs</span>
