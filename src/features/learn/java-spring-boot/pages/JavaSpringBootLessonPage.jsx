...
const READ_GATE_PREFIX = "java_spring_boot";

export default function JavaSpringBootLessonPage() {
  ...
  const lesson = JAVA_SPRING_BOOT_LESSONS.find((l) => l.id === lessonId);
  const lessonIdx = JAVA_SPRING_BOOT_LESSONS.findIndex(
    (l) => l.id === lessonId,
  );
  const prev = JAVA_SPRING_BOOT_LESSONS[lessonIdx - 1];
  const next = JAVA_SPRING_BOOT_LESSONS[lessonIdx + 1];
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        {/* ── Content ── */}
        <LessonContentShell
          storageKey={`java-spring-boot:${lessonId}`}
