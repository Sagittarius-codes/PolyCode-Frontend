...
import {
  JAVA_SPRING_BOOT_CHAPTERS,
  JAVA_SPRING_BOOT_LESSONS,
  JAVA_SPRING_BOOT_TOTAL_XP,
} from "../data/javaSpringBootCurriculum";
import useJavaSpringBootProgress from "../hooks/useJavaSpringBootProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/java-spring-boot";

const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["spring-basics"],
    color: "#22c55e",
    summary: "What is Spring Boot, layered architecture, and configuration.",
  },
  {
    level: "Intermediate",
    chapters: ["rest-controllers"],
    color: "#3b82f6",
    summary: "@RestController, @GetMapping, @PostMapping, and query params.",
  },
  {
    level: "Advanced",
    chapters: ["dependency-injection"],
    color: "#f59e0b",
    summary: "Beans, @Autowired constructor injection, and interfaces.",
  },
  {
    level: "Pro",
    chapters: ["data-persistence"],
    color: "#8b5cf6",
    summary: "JPA entities, Spring Data repositories, and DTOs.",
  },
];
...
export default function JavaSpringBootHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useJavaSpringBootProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = JAVA_SPRING_BOOT_LESSONS.filter(
    (l) => progress[l.id],
  ).reduce((sum, l) => sum + l.xp, 0);
  const pct =
    Math.round((completedCount / JAVA_SPRING_BOOT_LESSONS.length) * 100) || 0;

  const nextLesson =
    JAVA_SPRING_BOOT_LESSONS.find((l) => !progress[l.id]) ||
    JAVA_SPRING_BOOT_LESSONS[0];
  const resumeLesson =
    JAVA_SPRING_BOOT_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = JAVA_SPRING_BOOT_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => JAVA_SPRING_BOOT_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return JAVA_SPRING_BOOT_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub js-fundamentals-hub">
      ...
      <div className="oops-hero js-fundamentals-hero">
        ...
        <div className="oops-hero-badge">JAVA · SPRING BOOT</div>
        <h1 className="oops-hero-title">
          Java
          <br />
          <span className="oops-hero-accent" style={{ color: "#22c55e" }}>
            Spring Boot
          </span>
        </h1>
        <p className="oops-hero-sub">
          Master REST controllers, dependency injection, services, and
          Spring Data JPA with theory, quizzes, and real coding
          challenges compiled by{" "}
          <code>javac</code>.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${JAVA_SPRING_BOOT_LESSONS.length} lessons · ${earnedXP}/${JAVA_SPRING_BOOT_TOTAL_XP} XP`
                  : `Sign in to track progress · ${JAVA_SPRING_BOOT_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{JAVA_SPRING_BOOT_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{JAVA_SPRING_BOOT_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{JAVA_SPRING_BOOT_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <div className="oops-path-overview">
        {JAVA_SPRING_BOOT_CHAPTERS.map((chapter, index) => {
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {JAVA_SPRING_BOOT_CHAPTERS.length} chapters ·{" "}
            {JAVA_SPRING_BOOT_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = JAVA_SPRING_BOOT_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={JAVA_SPRING_BOOT_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        ...
        totalLessons={JAVA_SPRING_BOOT_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={JAVA_SPRING_BOOT_TOTAL_XP}
      />
    </div>
  );
}
