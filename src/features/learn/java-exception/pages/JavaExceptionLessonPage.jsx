...
const READ_GATE_PREFIX = "java_exception";

export default function JavaExceptionLessonPage() {
  ...
  const lesson = JAVA_EXCEPTION_LESSONS.find((l) => l.id === lessonId);
  const lessonIdx = JAVA_EXCEPTION_LESSONS.findIndex(
    (l) => l.id === lessonId,
  );
  const prev = JAVA_EXCEPTION_LESSONS[lessonIdx - 1];
  const next = JAVA_EXCEPTION_LESSONS[lessonIdx + 1];
    ...
    return (
      <div className="oops-not-found">
        ...
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to Java Exception Handling
        </button>
      </div>
    );
  }
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Java Exception Handling"
            syncLabel={
              isAuthenticated
        ...
        {/* ── Content ── */}
        <LessonContentShell
          storageKey={`java-exception:${lessonId}`}
