...
import {
  JAVA_EXCEPTION_CHAPTERS,
  JAVA_EXCEPTION_LESSONS,
  JAVA_EXCEPTION_TOTAL_XP,
} from "../data/javaExceptionCurriculum";
import useJavaExceptionProgress from "../hooks/useJavaExceptionProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/java-exception";

const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["exception-basics"],
    color: "#ef4444",
    summary: "What are exceptions, try-catch-finally, throw and throws.",
  },
  {
    level: "Intermediate",
    chapters: ["custom-exceptions"],
    color: "#f59e0b",
    summary: "Creating custom exceptions and exception chaining.",
  },
  {
    level: "Advanced",
    chapters: ["advanced-exceptions"],
    color: "#3b82f6",
    summary: "try-with-resources, best practices, exceptions in streams.",
  },
  {
    level: "Pro",
    chapters: ["exception-patterns"],
    color: "#8b5cf6",
    summary: "Validation pattern, Result pattern, stack traces, order processor.",
  },
];
...
export default function JavaExceptionHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useJavaExceptionProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = JAVA_EXCEPTION_LESSONS.filter(
    (l) => progress[l.id],
  ).reduce((sum, l) => sum + l.xp, 0);
  const pct =
    Math.round((completedCount / JAVA_EXCEPTION_LESSONS.length) * 100) || 0;

  const nextLesson =
    JAVA_EXCEPTION_LESSONS.find((l) => !progress[l.id]) ||
    JAVA_EXCEPTION_LESSONS[0];
  const resumeLesson =
    JAVA_EXCEPTION_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = JAVA_EXCEPTION_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => JAVA_EXCEPTION_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return JAVA_EXCEPTION_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub js-fundamentals-hub">
      ...
      <div className="oops-hero js-fundamentals-hero">
        ...
        <div className="oops-hero-badge">JAVA · EXCEPTION HANDLING</div>
        <h1 className="oops-hero-title">
          Java
          <br />
          <span className="oops-hero-accent" style={{ color: "#ef4444" }}>
            Exception Handling
          </span>
        </h1>
        <p className="oops-hero-sub">
          Master try-catch-finally, custom exceptions, try-with-resources,
          and exception-handling patterns with theory, quizzes, and real
          coding challenges compiled by{" "}
          <code>javac</code>.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${JAVA_EXCEPTION_LESSONS.length} lessons · ${earnedXP}/${JAVA_EXCEPTION_TOTAL_XP} XP`
                  : `Sign in to track progress · ${JAVA_EXCEPTION_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{JAVA_EXCEPTION_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{JAVA_EXCEPTION_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{JAVA_EXCEPTION_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <div className="oops-path-overview">
        {JAVA_EXCEPTION_CHAPTERS.map((chapter, index) => {
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {JAVA_EXCEPTION_CHAPTERS.length} chapters ·{" "}
            {JAVA_EXCEPTION_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = JAVA_EXCEPTION_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={JAVA_EXCEPTION_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        courseName="Java Exception Handling"
        totalLessons={JAVA_EXCEPTION_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={JAVA_EXCEPTION_TOTAL_XP}
      />
    </div>
  );
}
