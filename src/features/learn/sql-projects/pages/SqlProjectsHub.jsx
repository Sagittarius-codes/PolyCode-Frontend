...
import {
  SQLPROJECTS_CHAPTERS,
  SQLPROJECTS_LESSONS,
  SQLPROJECTS_TOTAL_XP,
} from "../data/sqlProjectsCurriculum";
import useSqlProjectsProgress from "../hooks/useSqlProjectsProgress";
...
export default function SqlProjectsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useSqlProjectsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = SQLPROJECTS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / SQLPROJECTS_LESSONS.length) * 100) || 0;
  const nextLesson =
    SQLPROJECTS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    SQLPROJECTS_LESSONS[0];
  const resumeLesson =
    SQLPROJECTS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = SQLPROJECTS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => SQLPROJECTS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return SQLPROJECTS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub cpp-fundamentals-hub">
      <div className="oops-hero cpp-fundamentals-hero">
        ...
        <p className="oops-hero-sub">
          Master SQL Projects with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${SQLPROJECTS_LESSONS.length} lessons · ${earnedXP}/${SQLPROJECTS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${SQLPROJECTS_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{SQLPROJECTS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{SQLPROJECTS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{SQLPROJECTS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {SQLPROJECTS_CHAPTERS.length} chapters · {" "}
      {SQLPROJECTS_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = SQLPROJECTS_CHAPTERS.filter((ch) =>
