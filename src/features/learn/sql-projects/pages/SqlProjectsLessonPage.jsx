...
import SqlProjectsCodeChallenge from "../components/SqlProjectsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "sqlprojects";

export default function SqlProjectsLessonPage() {
  ...
  const lesson = SQLPROJECTS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SQLPROJECTS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SQLPROJECTS_LESSONS[lessonIdx - 1];
  const next = SQLPROJECTS_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = SQLPROJECTS_LESSONS.filter((item) => progress[item.id]).reduce(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sqlprojects:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — SQL Projects`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
