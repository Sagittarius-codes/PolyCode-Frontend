...
const READ_GATE_PREFIX = "php_sessions";

export default function PhpSessionsLessonPage() {
  ...
  const lesson = PHP_SESSIONS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = PHP_SESSIONS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = PHP_SESSIONS_LESSONS[lessonIdx - 1];
  const next = PHP_SESSIONS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "PHP Sessions & Cookies",
    language: "PHP",
    lesson,
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>PHP Sessions lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to PHP Sessions & Cookies
        </button>
      </div>
    );
  }
  ...
  const earnedXP = PHP_SESSIONS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PHP Sessions & Cookies"
            syncLabel={
              isAuthenticated
                ? "PHP Sessions progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`php-sessions:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PHP Sessions`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
