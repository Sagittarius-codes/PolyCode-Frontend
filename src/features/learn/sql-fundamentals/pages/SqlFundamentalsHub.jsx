...
import {
  SQLFUNDAMENTALS_CHAPTERS,
  SQLFUNDAMENTALS_LESSONS,
  SQLFUNDAMENTALS_TOTAL_XP,
} from "../data/sqlFundamentalsCurriculum";
import useSqlFundamentalsProgress from "../hooks/useSqlFundamentalsProgress";
...
export default function SqlFundamentalsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useSqlFundamentalsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = SQLFUNDAMENTALS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / SQLFUNDAMENTALS_LESSONS.length) * 100) || 0;
  const nextLesson =
    SQLFUNDAMENTALS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    SQLFUNDAMENTALS_LESSONS[0];
  const resumeLesson =
    SQLFUNDAMENTALS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = SQLFUNDAMENTALS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => SQLFUNDAMENTALS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return SQLFUNDAMENTALS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub cpp-fundamentals-hub">
      <div className="oops-hero cpp-fundamentals-hero">
        ...
        <p className="oops-hero-sub">
          Master SQL Fundamentals with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${SQLFUNDAMENTALS_LESSONS.length} lessons · ${earnedXP}/${SQLFUNDAMENTALS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${SQLFUNDAMENTALS_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{SQLFUNDAMENTALS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{SQLFUNDAMENTALS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{SQLFUNDAMENTALS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {SQLFUNDAMENTALS_CHAPTERS.length} chapters · {" "}
      {SQLFUNDAMENTALS_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = SQLFUNDAMENTALS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={SQLFUNDAMENTALS_CHAPTERS}
        progress={progress}
