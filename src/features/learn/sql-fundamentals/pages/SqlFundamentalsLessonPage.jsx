...
import SqlFundamentalsCodeChallenge from "../components/SqlFundamentalsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "sqlfundamentals";

export default function SqlFundamentalsLessonPage() {
  ...
  const lesson = SQLFUNDAMENTALS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SQLFUNDAMENTALS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SQLFUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = SQLFUNDAMENTALS_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = SQLFUNDAMENTALS_LESSONS.filter((item) => progress[item.id]).reduce(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sqlfundamentals:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — SQL Fundamentals`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
