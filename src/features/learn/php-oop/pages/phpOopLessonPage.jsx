...
const READ_GATE_PREFIX = "php_oop";

export default function PhpOopLessonPage() {
  ...
  const lesson = PHP_OOP_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = PHP_OOP_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = PHP_OOP_LESSONS[lessonIdx - 1];
  const next = PHP_OOP_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = PHP_OOP_LESSONS.filter((item) => progress[item.id]).reduce(
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            ...
            syncLabel={
              isAuthenticated
                ? "PHP OOP progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`php-oop:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PHP OOP`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
