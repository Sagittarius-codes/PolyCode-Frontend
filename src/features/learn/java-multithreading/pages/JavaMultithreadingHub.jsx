...
import {
  JAVA_MULTITHREADING_CHAPTERS,
  JAVA_MULTITHREADING_LESSONS,
  JAVA_MULTITHREADING_TOTAL_XP,
} from "../data/javaMultithreadingCurriculum";
import useJavaMultithreadingProgress from "../hooks/useJavaMultithreadingProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/java-multithreading";

const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["thread-basics"],
    color: "#3b82f6",
    summary: "What is a thread, sleep/join, and race conditions.",
  },
  {
    level: "Intermediate",
    chapters: ["synchronization"],
    color: "#8b5cf6",
    summary: "synchronized, volatile, and AtomicInteger.",
  },
  {
    level: "Advanced",
    chapters: ["executor-service"],
    color: "#22c55e",
    summary: "Thread pools, Future, and Callable.",
  },
  {
    level: "Pro",
    chapters: ["concurrent-patterns"],
    color: "#f59e0b",
    summary: "ConcurrentHashMap, BlockingQueue, CompletableFuture, and deadlocks.",
  },
];
...
export default function JavaMultithreadingHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useJavaMultithreadingProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = JAVA_MULTITHREADING_LESSONS.filter(
    (l) => progress[l.id],
  ).reduce((sum, l) => sum + l.xp, 0);
  const pct =
    Math.round((completedCount / JAVA_MULTITHREADING_LESSONS.length) * 100) || 0;

  const nextLesson =
    JAVA_MULTITHREADING_LESSONS.find((l) => !progress[l.id]) ||
    JAVA_MULTITHREADING_LESSONS[0];
  const resumeLesson =
    JAVA_MULTITHREADING_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = JAVA_MULTITHREADING_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => JAVA_MULTITHREADING_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return JAVA_MULTITHREADING_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub js-fundamentals-hub">
      ...
      <div className="oops-hero js-fundamentals-hero">
        ...
        <div className="oops-hero-badge">JAVA · MULTITHREADING</div>
        ...
        <p className="oops-hero-sub">
          Master threads, synchronization, executor services, and modern
          concurrent patterns with theory, quizzes, and real coding
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${JAVA_MULTITHREADING_LESSONS.length} lessons · ${earnedXP}/${JAVA_MULTITHREADING_TOTAL_XP} XP`
                  : `Sign in to track progress · ${JAVA_MULTITHREADING_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{JAVA_MULTITHREADING_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{JAVA_MULTITHREADING_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{JAVA_MULTITHREADING_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <div className="oops-path-overview">
        {JAVA_MULTITHREADING_CHAPTERS.map((chapter, index) => {
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {JAVA_MULTITHREADING_CHAPTERS.length} chapters ·{" "}
            {JAVA_MULTITHREADING_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = JAVA_MULTITHREADING_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={JAVA_MULTITHREADING_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        ...
        totalLessons={JAVA_MULTITHREADING_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={JAVA_MULTITHREADING_TOTAL_XP}
      />
    </div>
  );
}
