...
import JavaCodeChallenge from "../../java-fundamenta../../java-fundamentals/components/JavaCodeChallenge";
import {
...
const READ_GATE_PREFIX = "java_multithreading";

export default function JavaMultithreadingLessonPage() {
  ...
  const lesson = JAVA_MULTITHREADING_LESSONS.find((l) => l.id === lessonId);
  const lessonIdx = JAVA_MULTITHREADING_LESSONS.findIndex(
    (l) => l.id === lessonId,
  );
  const prev = JAVA_MULTITHREADING_LESSONS[lessonIdx - 1];
  const next = JAVA_MULTITHREADING_LESSONS[lessonIdx + 1];
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Java Multithreading"
            syncLabel={
              isAuthenticated
        ...
        {/* ── Content ── */}
        <LessonContentShell
          storageKey={`java-multithreading:${lessonId}`}
        ...
        >
          {tab === "theory" ? (
            ...
            <JavaCodeChallenge
              ...
              accentColor={lesson.chapterColor || "#e76f00"}
