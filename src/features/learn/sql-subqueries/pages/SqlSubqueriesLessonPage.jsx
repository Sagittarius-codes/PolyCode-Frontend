...
import SqlSubqueriesCodeChallenge from "../components/SqlSubqueriesCodeChallenge";
import {
...
const READ_GATE_PREFIX = "sqlsubqueries";

export default function SqlSubqueriesLessonPage() {
  ...
  const lesson = SQLSUBQUERIES_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SQLSUBQUERIES_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SQLSUBQUERIES_LESSONS[lessonIdx - 1];
  const next = SQLSUBQUERIES_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "SQL Subqueries",
    language: "SQL",
    lesson,
  ...
  const earnedXP = SQLSUBQUERIES_LESSONS.filter((item) => progress[item.id]).reduce(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sqlsubqueries:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — SQL Subqueries`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
