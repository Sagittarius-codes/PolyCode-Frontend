...
import {
  SQLSUBQUERIES_CHAPTERS,
  SQLSUBQUERIES_LESSONS,
  SQLSUBQUERIES_TOTAL_XP,
} from "../data/sqlSubqueriesCurriculum";
import useSqlSubqueriesProgress from "../hooks/useSqlSubqueriesProgress";
...
export default function SqlSubqueriesHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useSqlSubqueriesProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = SQLSUBQUERIES_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / SQLSUBQUERIES_LESSONS.length) * 100) || 0;
  const nextLesson =
    SQLSUBQUERIES_LESSONS.find((lesson) => !progress[lesson.id]) ||
    SQLSUBQUERIES_LESSONS[0];
  const resumeLesson =
    SQLSUBQUERIES_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = SQLSUBQUERIES_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => SQLSUBQUERIES_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return SQLSUBQUERIES_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub cpp-fundamentals-hub">
      <div className="oops-hero cpp-fundamentals-hero">
        ...
        <p className="oops-hero-sub">
          Master SQL Subqueries with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${SQLSUBQUERIES_LESSONS.length} lessons · ${earnedXP}/${SQLSUBQUERIES_TOTAL_XP} XP`
                  : `Sign in to track progress · ${SQLSUBQUERIES_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{SQLSUBQUERIES_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{SQLSUBQUERIES_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{SQLSUBQUERIES_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {SQLSUBQUERIES_CHAPTERS.length} chapters · {" "}
      {SQLSUBQUERIES_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = SQLSUBQUERIES_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={SQLSUBQUERIES_CHAPTERS}
        progress={progress}
