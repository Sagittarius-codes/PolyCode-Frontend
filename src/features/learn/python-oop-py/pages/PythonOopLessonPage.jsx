...
const READ_GATE_PREFIX = "python_oop_py";

export default function PythonOopLessonPage() {
  ...
  const lesson = PYTHON_OOP_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = PYTHON_OOP_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = PYTHON_OOP_LESSONS[lessonIdx - 1];
  const next = PYTHON_OOP_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Python OOP lesson not found.</p>
