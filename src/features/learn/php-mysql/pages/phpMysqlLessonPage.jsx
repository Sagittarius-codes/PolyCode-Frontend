...
const READ_GATE_PREFIX = "php_mysql";

export default function PhpMysqlLessonPage() {
  ...
  const lesson = PHP_MYSQL_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = PHP_MYSQL_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = PHP_MYSQL_LESSONS[lessonIdx - 1];
  const next = PHP_MYSQL_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>PHP MySQL lesson not found.</p>
  ...
  const earnedXP = PHP_MYSQL_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PHP MySQL"
            syncLabel={
              isAuthenticated
                ? "PHP MySQL progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`php-mysql:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PHP MySQL`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
