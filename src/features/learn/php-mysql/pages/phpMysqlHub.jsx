...
import {
  ...
  PHP_MYSQL_LESSONS,
  PHP_MYSQL_TOTAL_XP,
} from "../data/phpMysqlCurriculum";
import usePhpMysqlProgress from "../hooks/usePhpMysqlProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/php-mysql";

const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["connecting-pdo"],
    color: "#06b6d4",
    summary: "What PDO is, connecting safely, and building DSN strings.",
  },
  {
    level: "Intermediate",
    chapters: ["queries-prepared-statements"],
    color: "#3b82f6",
    summary: "Prepared statements, fetching results, and INSERT/UPDATE/DELETE.",
  },
  {
    level: "Advanced",
    chapters: ["transactions-error-handling"],
    color: "#f59e0b",
    summary: "Catching PDOException, transactions, and constraint violations.",
  },
  {
    level: "Pro",
    chapters: ["dao-pattern-real-apps"],
    color: "#8b5cf6",
    summary: "The Repository pattern, dependency injection, and pagination.",
  },
];
...
export default function PhpMysqlHub() {
  ...
  const earnedXP = PHP_MYSQL_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / PHP_MYSQL_LESSONS.length) * 100) || 0;
  const nextLesson =
    PHP_MYSQL_LESSONS.find((lesson) => !progress[lesson.id]) ||
    PHP_MYSQL_LESSONS[0];
  const resumeLesson =
    PHP_MYSQL_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = PHP_MYSQL_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => PHP_MYSQL_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return PHP_MYSQL_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub php-mysql-hub">
      <div className="oops-hero php-mysql-hero">
        <Link
          to="/language/PHP"
        ...
        <div className="oops-hero-badge">PHP · MYSQL</div>
        <h1 className="oops-hero-title">
          PHP
          <br />
          <span className="oops-hero-accent" style={{ color: "#06b6d4" }}>MySQL</span>
        </h1>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${PHP_MYSQL_LESSONS.length} lessons · ${earnedXP}/${PHP_MYSQL_TOTAL_XP} XP`
                  : `Sign in to track progress · ${PHP_MYSQL_LESSONS.length} lessons`}
              </span>
            ...
            <div className="oops-xp-track">
              <div
                ...
                style={{ width: isAuthenticated ? `${pct}%` : "0%", backgroundColor: "#06b6d4" }}
              />
            </div>
          </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a PHP MySQL topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search PDO, queries, transactions..."
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{PHP_MYSQL_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{PHP_MYSQL_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{PHP_MYSQL_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {PHP_MYSQL_CHAPTERS.length} chapters ·{" "}
            {PHP_MYSQL_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = PHP_MYSQL_CHAPTERS.filter((ch) =>
      ...
      <CourseCertificate
        ...
        totalLessons={PHP_MYSQL_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={PHP_MYSQL_TOTAL_XP}
      />
    </div>
  );
}