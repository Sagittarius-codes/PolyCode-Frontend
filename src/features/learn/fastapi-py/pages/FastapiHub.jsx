...
export default function FastapiHub() {
  ...
  const earnedXP = FASTAPI_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct = Math.round((completedCount / FASTAPI_LESSONS.length) * 100) || 0;
  const nextLesson =
    FASTAPI_LESSONS.find((lesson) => !progress[lesson.id]) || FASTAPI_LESSONS[0];
  const resumeLesson =
    FASTAPI_LESSONS.find((lesson) => lesson.id === lastLessonId) || nextLesson;
  const completedChapters = FASTAPI_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => FASTAPI_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return FASTAPI_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub fastapi-hub">
      <div className="oops-hero fastapi-hero">
        <Link
          to="/language/Python"
        ...
        <div className="oops-hero-badge">FASTAPI · PYTHON TRACK</div>
        ...
        <p className="oops-hero-sub">
          Build modern REST APIs with Python — HTTP basics, Pydantic validation,
          CRUD patterns, dependencies, routers, testing, and a full capstone
          project.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${FASTAPI_LESSONS.length} lessons · ${earnedXP}/${FASTAPI_TOTAL_XP} XP`
                  : `Sign in to track progress · ${FASTAPI_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume FastAPI" : "Start FastAPI"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a FastAPI topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search routes, Pydantic, CRUD, TestClient..."
              aria-label="Search FastAPI lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter FastAPI lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{FASTAPI_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{FASTAPI_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{FASTAPI_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
