...
export default function CDataStructuresHub() {
  ...
  const earnedXP = C_DATA_STRUCTURES_LESSONS.filter((l) => progress[l.id]).reduce(
    (sum, l) => sum + l.xp,
    0,
  );
  const pct =
    Math.round((completedCount / C_DATA_STRUCTURES_LESSONS.length) * 100) || 0;
  const nextLesson =
    C_DATA_STRUCTURES_LESSONS.find((l) => !progress[l.id]) || C_DATA_STRUCTURES_LESSONS[0];
  const resumeLesson =
    C_DATA_STRUCTURES_LESSONS.find((l) => l.id === lastLessonId) || nextLesson;
  const completedChapters = C_DATA_STRUCTURES_CHAPTERS.filter((ch) =>
    ch.lessons.every((l) => progress[l.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => C_DATA_STRUCTURES_LESSONS.find((l) => l.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return C_DATA_STRUCTURES_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub c-hub">
      <div className="oops-hero c-hero" style={{ borderColor: "#2980b9" }}>
        <Link
          to="/language/C"
        ...
        <h1 className="oops-hero-title">
          C Data
          <br />
          <span className="oops-hero-accent" style={{ color: "#2980b9" }}>
            Structures
          </span>
        </h1>
        <p className="oops-hero-sub">Arrays, linked lists, stacks, queues, trees, and sorting — implement classic structures in pure C.</p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${C_DATA_STRUCTURES_LESSONS.length} lessons · ${earnedXP}/${C_DATA_STRUCTURES_TOTAL_XP} XP`
                  : `Sign in to track progress · ${C_DATA_STRUCTURES_LESSONS.length} lessons`}
              </span>
            ...
            <div className="oops-xp-track">
              <div
                ...
                style={{ width: isAuthenticated ? `${pct}%` : "0%", backgroundColor: "#2980b9" }}
              />
            </div>
          </div>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C Data Structures" : "Start C Data Structures"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search linked list, stack, tree..."
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>{completedCount}/{C_DATA_STRUCTURES_LESSONS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>{completedChapters}/{C_DATA_STRUCTURES_CHAPTERS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>{earnedXP}/{C_DATA_STRUCTURES_TOTAL_XP}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
