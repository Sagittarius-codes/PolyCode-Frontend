...
import {
  ...
  SQLINDEXES_LESSONS,
  SQLINDEXES_TOTAL_XP,
} from "../data/sqlIndexesCurriculum";
import useSqlIndexesProgress from "../hooks/useSqlIndexesProgress";
...
export default function SqlIndexesHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useSqlIndexesProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = SQLINDEXES_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / SQLINDEXES_LESSONS.length) * 100) || 0;
  const nextLesson =
    SQLINDEXES_LESSONS.find((lesson) => !progress[lesson.id]) ||
    SQLINDEXES_LESSONS[0];
  const resumeLesson =
    SQLINDEXES_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = SQLINDEXES_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => SQLINDEXES_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return SQLINDEXES_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub cpp-fundamentals-hub">
      <div className="oops-hero cpp-fundamentals-hero">
        ...
        <p className="oops-hero-sub">
          Master SQL Indexes with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${SQLINDEXES_LESSONS.length} lessons · ${earnedXP}/${SQLINDEXES_TOTAL_XP} XP`
                  : `Sign in to track progress · ${SQLINDEXES_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{SQLINDEXES_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{SQLINDEXES_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{SQLINDEXES_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {SQLINDEXES_CHAPTERS.length} chapters · {" "}
      {SQLINDEXES_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = SQLINDEXES_CHAPTERS.filter((ch) =>
