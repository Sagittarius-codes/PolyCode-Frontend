...
import SqlIndexesCodeChallenge from "../components/SqlIndexesCodeChallenge";
import {
  SQLINDEXES_CHAPTERS,
  SQLINDEXES_LESSONS,
  SQLINDEXES_TOTAL_XP,
...
const READ_GATE_PREFIX = "sqlindexes";

export default function SqlIndexesLessonPage() {
  ...
  const lesson = SQLINDEXES_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SQLINDEXES_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SQLINDEXES_LESSONS[lessonIdx - 1];
  const next = SQLINDEXES_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = SQLINDEXES_LESSONS.filter((item) => progress[item.id]).reduce(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sqlindexes:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — SQL Indexes`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
