...
import {
  SQLSTOREDPROCEDURES_CHAPTERS,
  SQLSTOREDPROCEDURES_LESSONS,
  SQLSTOREDPROCEDURES_TOTAL_XP,
} from "../data/sqlStoredProceduresCurriculum";
import useSqlStoredProceduresProgress from "../hooks/useSqlStoredProceduresProgress";
...
export default function SqlStoredProceduresHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useSqlStoredProceduresProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = SQLSTOREDPROCEDURES_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / SQLSTOREDPROCEDURES_LESSONS.length) * 100) || 0;
  const nextLesson =
    SQLSTOREDPROCEDURES_LESSONS.find((lesson) => !progress[lesson.id]) ||
    SQLSTOREDPROCEDURES_LESSONS[0];
  const resumeLesson =
    SQLSTOREDPROCEDURES_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = SQLSTOREDPROCEDURES_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => SQLSTOREDPROCEDURES_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return SQLSTOREDPROCEDURES_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub cpp-fundamentals-hub">
      <div className="oops-hero cpp-fundamentals-hero">
        ...
        <p className="oops-hero-sub">
          Master SQL Stored Procedures with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${SQLSTOREDPROCEDURES_LESSONS.length} lessons · ${earnedXP}/${SQLSTOREDPROCEDURES_TOTAL_XP} XP`
                  : `Sign in to track progress · ${SQLSTOREDPROCEDURES_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{SQLSTOREDPROCEDURES_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{SQLSTOREDPROCEDURES_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{SQLSTOREDPROCEDURES_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {SQLSTOREDPROCEDURES_CHAPTERS.length} chapters · {" "}
      {SQLSTOREDPROCEDURES_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = SQLSTOREDPROCEDURES_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={SQLSTOREDPROCEDURES_CHAPTERS}
        progress={progress}
