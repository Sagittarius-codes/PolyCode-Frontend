...
import SqlStoredProceduresCodeChallenge from "../components/SqlStoredProceduresCodeChallenge";
import {
...
const READ_GATE_PREFIX = "sqlstoredprocedures";

export default function SqlStoredProceduresLessonPage() {
  ...
  const lesson = SQLSTOREDPROCEDURES_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SQLSTOREDPROCEDURES_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SQLSTOREDPROCEDURES_LESSONS[lessonIdx - 1];
  const next = SQLSTOREDPROCEDURES_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "SQL Stored Procedures",
    language: "SQL",
    lesson,
  ...
  const earnedXP = SQLSTOREDPROCEDURES_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="SQL Stored Procedures"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sqlstoredprocedures:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — SQL Stored Procedures`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <SqlStoredProceduresCodeChallenge
