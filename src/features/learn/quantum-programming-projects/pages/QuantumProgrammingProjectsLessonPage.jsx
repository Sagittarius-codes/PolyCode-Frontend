...
const READ_GATE_PREFIX = "quantum_programming_projects";

export default function QuantumProgrammingProjectsLessonPage() {
  ...
  const lesson = QUANTUM_PROGRAMMING_PROJECTS_LESSONS.find(
    (item) => item.id === lessonId,
  );
  const lessonIdx = QUANTUM_PROGRAMMING_PROJECTS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = QUANTUM_PROGRAMMING_PROJECTS_LESSONS[lessonIdx - 1];
  const next = QUANTUM_PROGRAMMING_PROJECTS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Quantum Programming Projects",
    language: "Quantum",
    lesson,
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Quantum Programming Projects lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to Quantum Programming Projects
        </button>
      </div>
    );
  }
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Quantum Programming Projects"
            syncLabel={
              isAuthenticated
                ? "Quantum Projects progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`quantum-programming-projects:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Quantum Programming Projects`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
