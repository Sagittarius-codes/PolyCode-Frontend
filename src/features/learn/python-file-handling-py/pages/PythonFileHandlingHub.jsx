...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["foundations", "reading-writing"],
    color: "#2563eb",
    summary: "Why files matter, open/read/write, and line-by-line processing.",
  },
  {
    level: "Intermediate",
    chapters: ["paths-modes", "csv-data", "json-config"],
    color: "#059669",
    summary: "pathlib, encoding, CSV, JSON, and config patterns.",
  },
  {
    level: "Advanced",
    chapters: ["errors-safety", "advanced-io"],
    color: "#dc2626",
    summary: "Safe I/O, shutil, glob, binary files, and StringIO.",
  },
  {
    level: "Pro",
    chapters: ["pro-workflows"],
    color: "#4f46e5",
    summary: "Logging, context managers, ETL capstone, and cheat sheet.",
  },
];
...
export default function PythonFileHandlingHub() {
  ...
  const pct =
    Math.round((completedCount / PYTHON_FILE_HANDLING_LESSONS.length) * 100) || 0;
  const nextLesson =
    PYTHON_FILE_HANDLING_LESSONS.find((lesson) => !progress[lesson.id]) ||
    PYTHON_FILE_HANDLING_LESSONS[0];
  const resumeLesson =
    PYTHON_FILE_HANDLING_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = PYTHON_FILE_HANDLING_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => PYTHON_FILE_HANDLING_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return PYTHON_FILE_HANDLING_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub matplotlib-hub python-file-handling-hub">
      <div className="oops-hero matplotlib-hero python-file-handling-hero">
        <Link
          to="/language/Python"
        ...
        <div className="oops-hero-badge">FILE HANDLING · BEGINNER → PRO</div>
        <h1 className="oops-hero-title">
          Python File
          <br />
          <span className="oops-hero-accent">Handling</span>
        </h1>
        <p className="oops-hero-sub">
          A structured path from your first `open()` to production-safe CSV,
          JSON, and pathlib workflows — 8 chapters, 25 lessons, and hands-on
          challenges that teach durable I/O the way real Python projects do it.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${PYTHON_FILE_HANDLING_LESSONS.length} lessons · ${earnedXP}/${PYTHON_FILE_HANDLING_TOTAL_XP} XP`
                  : `Sign in to track progress · ${PYTHON_FILE_HANDLING_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume File Handling" : "Start File Handling"}
            </button>
          </div>
        </div>
      </div>
      ...
      <section className="matplotlib-prerequisites" aria-label="Before you start">
        ...
        <div className="matplotlib-prerequisites-grid">
          <Link to="/learn/python-fundamentals" className="matplotlib-prereq-card">
            ...
            <p>Variables, functions, and basic `with open()` preview.</p>
          </Link>
          <Link to="/learn/pandas-py" className="matplotlib-prereq-card">
            <strong>Pandas</strong>
            <p>Load CSV/DataFrames after mastering raw file I/O here.</p>
          </Link>
          <Link
            to={`${BASE_PATH}/lesson/fh-15`}
            className="matplotlib-prereq-card matplotlib-prereq-cheat"
          >
            <strong>Cheat sheet</strong>
            <p>File I/O quick reference — bookmark lesson 25 anytime.</p>
          </Link>
        </div>
        <p className="matplotlib-runtime-note">
          Challenges run in your browser (Pyodide) or on the server. Many use
          <code> io.StringIO</code> so you can practice file logic without
          writing to disk.
        </p>
      </section>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a file I/O topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search pathlib, csv, json, atomic write..."
              aria-label="Search file handling lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter file handling lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{PYTHON_FILE_HANDLING_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{PYTHON_FILE_HANDLING_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{PYTHON_FILE_HANDLING_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {PYTHON_FILE_HANDLING_CHAPTERS.length} chapters ·{" "}
            {PYTHON_FILE_HANDLING_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = PYTHON_FILE_HANDLING_CHAPTERS.filter((ch) =>
